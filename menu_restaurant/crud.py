...
def create_menu(db: Session, menu: schemas.MenusCreate):
    db_menu = models.Menus(
        id=str(uuid.uuid4()),
        title=menu.title,
...
def update_menu(db: Session, menus_id: str,
...
def create_submenu(db: Session,
    ...
    db_menu_submenu = (db.query(models.Submenus)
                       .filter(models.Submenus.target_menu_id == menus_id)
                       .one_or_none()
                       )
    db_menu_submenu = (models.Submenus(target_menu_id=menus_id,
                                       id=str(uuid.uuid4()),
                                       title=submenu.title,
