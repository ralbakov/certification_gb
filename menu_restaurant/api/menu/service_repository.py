...
def update_menu_service(target_menu_id: str,
    ...
    """Обновляет меню в кэше"""

    if menu is None:
        raise HTTPException(status_code=404, detail='menu not found')
    elif menu is ValueError:
        raise HTTPException(status_code=409, detail='menu not update, such parameters already exist')
